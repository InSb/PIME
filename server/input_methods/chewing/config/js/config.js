...
function saveConfig() {
    ...
    $.post(apiUrl + "save/config",
        { data: str }
    );

    if(symbolsChanged) {
        str = $("#symbols").val();
        $.post(apiUrl + "save/symbols",
            { data: str }
        );
    }
    if(swkbChanged) {
        str = $("#ez_symbols").val();
        $.post(apiUrl + "save/swkb",
            { data: str }
        );
    }
}
...
function initAPIUrl() {
    ...
    var command = ChewingConfig.commandLine;
    var sep = command.lastIndexOf(' ');
    var port = "5566";
    if(sep >= 0)
        port = command.substr(sep + 1);
